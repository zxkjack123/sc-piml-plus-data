...
| 20-split robustness (`14/20`, seeds 100-119) | **not reproducible; re-run disagrees** | The runs were never retained (the split-robustness runs on disk, `p5_split_robust_r00..r19_*`, are N80/51-13-16 with `cv_folds=0`, not the 149/31/44 N=224 study described; `d224_stability_max/` keeps only seed 100's tables). Re-run with the recovered harness gives **2/20 splits, mean 1.15 +/- 1.01, no 4/4 seed**, against the published 14/20, 3.0 +/- 0.8 and seven named 4/4 seeds. The published headline is only reached at a threshold about 10 pp looser (0.80) than the stated 90%. See `docs/ablation_harness.md` section 8 and `tables/split_robust_20_*.csv`. |
